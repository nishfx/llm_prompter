...
# Add this block here (at the very top)
def get_config_path():
    # Check if running as a PyInstaller binary
    if hasattr(sys, '_MEIPASS'):
        # Running from PyInstaller binary, config.json will be bundled
        base_path = sys._MEIPASS
    else:
        # Running from source, use the script's directory
        base_path = os.path.dirname(os.path.abspath(__file__))

    # Use the config.json in the same directory
...
FILE_EXTENSIONS = ('.py', '.txt', '.md', '.json', '.yaml', '.yml', '.js', '.css', '.html')
...
class PromptBuilder(QWidget):
    ...
    **Below** that, there is an "Additional Questions" section with 3 question checkboxes.
    """
    def __init__(self, parent_app):
        super().__init__()
        self.parent_app = parent_app

        # For normal snippet categories:
        self.selected_snippets = {}

        # For “common questions”:
        self.questions_selected = set()
        ...
        # A scroll area for the horizontally-arranged categories
        ...
        # ----------------------------------------------------------------
        # Additional Questions Section - below categories, vertically
        # ----------------------------------------------------------------
        questions_group = QGroupBox("Additional Questions")
        questions_layout = QVBoxLayout(questions_group)

        # We'll store the text of each question in a dictionary: question->checkbox
        ...
        for qtext in self.common_questions:
            q_cb = QCheckBox(qtext)
            q_cb.stateChanged.connect(partial(self.on_question_checkbox_changed, qtext))
            questions_layout.addWidget(q_cb)
            self.question_checkboxes[qtext] = q_cb
    ...
    def on_checkbox_changed(self, category, item_name, state):
        """
        Called when the user toggles a snippet from the horizontal categories.
        """

        # '2' means fully checked, '0' means unchecked
        checked = (state == 2)
        if checked:
            if item_name == "Custom":
    ...
    def on_question_checkbox_changed(self, question_text, state):
        """
        Called when the user toggles one of the "Additional Questions" checkboxes.
        """

        checked = (state == 2)
        if checked:
            self.questions_selected.add(question_text)
        else:
            if question_text in self.questions_selected:
                self.questions_selected.remove(question_text)
    ...
    def build_instructions_xml(self):
        ...
        if self.questions_selected:
            lines.append("    <questions>")
            for qtext in self.questions_selected:
...
class FileTab(QFrame):
    """
    Exactly the same code as before for handling files/folders, no changes needed.
    """
...
class FileContentApp(QMainWindow):
    ...
    right side: vertical splitter => top = prompt builder, bottom = text area.
    """
    def __init__(self):
        super().__init__()
    ...
    def clear_content(self):
        """Uncheck all items in all tabs, uncheck prompt builder (snippets + questions), clear text."""
        self.content_text.clear()

        # 1) Uncheck all file items
        ...
        # 2) Uncheck prompt builder categories
        ...
        # 3) Uncheck additional question checkboxes
        for qtext, q_cb in self.prompt_builder.question_checkboxes.items():
            q_cb.blockSignals(True)
            q_cb.setChecked(False)
            q_cb.blockSignals(False)

        self.prompt_builder.questions_selected.clear()
    ...
    def update_context_in_real_time(self):
        """Rebuild the final prompt text area from instructions + context."""
        self.content_text.clear()

        instructions_block = self.prompt_builder.build_instructions_xml()

        unique_files = set()
        # Gather all checked files from each tab
        for i in range(self.tabs.count()):
            tab_widget = self.tabs.widget(i)
